...
public class BusinessAccountController extends AccountController {
	...
	public static BusinessAccountModel createAccountModel(String name) {
		...
		// Prepare and run sql
		sql = "SELECT ContactNo, Name, Address, Email " + "FROM Accounts " + "WHERE Username=?;";
		...
		BusinessAccountModel acc = new BusinessAccountModel(name, busname, contactno, address, email);
		dbCont.closeConnection();
		return acc;
	}
